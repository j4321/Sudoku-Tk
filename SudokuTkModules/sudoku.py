...
class Sudoku(Tk):
    def __init__(self):
        ...
        # images
        self.im_erreur = ImageTk.PhotoImage(Image.open(ERREUR))
        self.im_pause = ImageTk.PhotoImage(Image.open(PAUSE))
        self.im_play = ImageTk.PhotoImage(Image.open(PLAY))
        self.im_info = ImageTk.PhotoImage(Image.open(INFO))
        self.im_undo = ImageTk.PhotoImage(Image.open(UNDO))
        self.im_redo = ImageTk.PhotoImage(Image.open(REDO))
        self.im_question = ImageTk.PhotoImage(Image.open(QUESTION))
        ...
        self.tps.grid(row=1, column=0, sticky="e", padx=10, pady=(30,30))
        self.b_pause.grid(row=1, column=1, sticky="w", padx=10, pady=(30,30))

        # Retour en arrière
        self.b_undo = Button(self, image=self.im_undo, command=self.undo)
        self.b_undo.grid(row=1, column=2, sticky="e", pady=(30,30), padx=2)

        self.b_redo = Button(self, image=self.im_redo, command=self.redo)
        self.b_redo.grid(row=1, column=3, sticky="w", pady=(30,30), padx=2)

        # frame contenant la grille de sudoku
        self.frame = Frame(self, style="bg.TFrame")
        self.frame.grid(row=0, columnspan=4, padx=30, pady=(30,0))

        # menu
        self.menu = Menu(self, tearoff=0)

        menu_nouveau = Menu(self.menu, tearoff=0)
        menu_nouveau.add_command(label=_("Generate a puzzle"), command=self.genere_grille,
                                 accelerator="Ctrl+G")
        menu_nouveau.add_command(label=_("Empty grid"), command=self.grille_vide,
                                 accelerator="Ctrl+N")

        menu_ouvrir = Menu(self.menu, tearoff=0)
        ...
        self.menu.add_cascade(label=_("New"), menu=menu_nouveau)
        self.menu.add_cascade(label=_("Open"), menu=menu_ouvrir)
        self.menu.add_command(label=_("Save"), command=self.sauvegarde)
        self.menu.add_command(label=_("Restart"), command=self.recommence)
        self.menu.add_command(label=_("Solve"), command=self.resolution)
        self.menu.add_command(label=_("Export"), command=self.export_impression)
        self.menu.add_cascade(label=_("Help"), menu=menu_help)
        ...
        self.bind("<Control-z>", lambda e: self.undo(), add=True)
        self.bind("<Control-y>", lambda e: self.redo(), add=True)
        self.bind("<Control-s>", lambda e: self.sauvegarde(), add=True)
        self.bind("<Control-e>", lambda e: self.export_impression(), add=True)
        self.bind("<Control-o>", lambda e: self.import_partie(), add=True)
        self.bind("<Control-Shift-O>", lambda e: self.import_grille(), add=True)
        self.bind("<Control-n>", lambda e: self.grille_vide(), add=True)
        self.bind("<Control-g>", lambda e: self.genere_grille(), add=True)
        self.bind("<FocusOut>", self.focus_out, add=True)
        ...
        if self.nb_cases_remplies == 81:
            ...
            if type(sol) == np.ndarray:
                ...
                one_button_box(self, _("Information"),
                               ...
                               style=STYLE, image=self.im_info)
                ...
                one_button_box(self, _("Information"), _("There is a mistake."),
                               image=self.im_info, style=STYLE)
    ...
    def grille_vide(self):
        ...
        if self.debut:
            rep = two_button_box(self, _("Confirmation"),
                                    _("Do you want to abandon the current puzzle?"),
                                    _("Yes"), _("No"), self.im_question, style=STYLE)
        if rep == _("Yes"):
            self.nb_cases_remplies = 0
            self.restart()
            for i in range(9):
    ...
    def genere_grille(self):
        ...
        if rep == _("Yes"):
            ...
            while rep2 == _("Retry"):
                ...
                diff = dico[difficulte_grille(grille)]
                nb = grille.nb_cases_remplies()
                self.configure(cursor="")
                rep2 = two_button_box(self, _("Information"),
                                     _("The generated puzzle contains %(nb)i numbers and its level is %(difficulty)s.") % ({"nb": nb, "difficulty":diff}),
                                     _("Retry"), _("Play"), image=self.im_info, style=STYLE)
            if rep2 == _("Play"):
    ...
    def resolution_init(self):
        ...
        if type(sol) == np.ndarray:
            ...
            one_button_box(self, _("Error"), _("The grid is wrong. It cannot be solved."),
                           image=self.im_erreur, style=STYLE)
        else:
            one_button_box(self, _("Error"), _("Resolution failed."), style=STYLE,
    ...
    def resolution(self):
        ...
        rep = two_button_box(self, _("Confirmation"),
                             ...
                             _("Yes"), _("No"), image=self.im_question, style=STYLE)
        if rep == _("Yes"):
            grille = Grille()
            for i in range(9):
                for j in range(9):
            ...
            if type(sol) == np.ndarray:
                ...
                if i < 9:
                    ...
                    rep = two_button_box(self, _("Error"),
                                         ...
                                         _("Yes"), _("No"), style=STYLE, image=self.im_erreur)
                    if rep == _("Yes"):
    ...
    def import_grille(self):
        ...
        if self.debut:
            rep = two_button_box(self, _("Confirmation"),
                                 ...
                                 _("Yes"), _("No"), self.im_question, style=STYLE)
        if rep == _("Yes"):
            chemin = join(INITIALDIR, "Puzzles")
            if not exists(chemin):
                chemin = INITIALDIR
            fichier = askopenfilename(initialdir=chemin,
